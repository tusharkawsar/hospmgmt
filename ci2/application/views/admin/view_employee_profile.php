
        <div class="left_side_bar"> 
          <? php echo anchor('doc/view_patient_info','See Patient Info');?>
          <div class="col_1">
            <h1>Genaral Menu</h1>
            <div class="box">
              <ul>
                <li><?php echo anchor('admin/index','Home');?></li></br>

                <li><?php echo anchor('admin/edit_info','Edit Info');?></li></br>
                <li><?php echo anchor('admin/view_schedule','View Schedule');?></li></br>
              </ul>
            </div>
          </div>
          
          <div class="col_1">
            <h1>Employee Menu</h1>
            <div class="box">
              <ul>
                <li><?php echo anchor('admin/create_employee_account','Create Employee Account');?></li></br>  
                <li><?php echo anchor('admin/view_employee_profile','View Employee Profile');?></li></br>
                <li><?php echo anchor('admin/view_patient_profile','View Patient Profile');?></li></br>
                <li><?php echo anchor('admin/view_bed_status','View Bed Status');?></li></br>
                <li><?php echo anchor('admin/view_employee_schedule','View Employee Schedule');?></li></br>             
                <li><?php echo anchor('admin/change_password','Change Password');?></li></br>
                <li><?php echo anchor('admin/log_out','Log Out');?></li></br>
              </ul>
            </div>
          </div>

        </div>

        <div class="right_section">
          <div class="common_content">
            <?php

                $doc = $this->input->post('doctors');
                $nur = $this->input->post('nurses');
                $emp = $this->input->post('emp');
                echo $doc." ".$nur." ".$emp;
                echo form_open('admin/view_employee_profile');
                echo form_label("Doctors:",'');
                echo form_dropdown('doctors', $doclist,$doclist[0]);
                echo form_submit('SubmitD','Search DOC');
                echo form_open('admin/view_employee_profile');
                echo form_label("Nurses:",'');
                echo form_dropdown('nurses', $nurselist,$nurselist[0]);
                echo form_submit('SubmitN','Search Nurse');
                echo form_open('admin/view_employee_profile');
                echo form_label("Employees:",'');
                echo form_dropdown('emp', $emplist,$emplist[0]);
                echo form_submit('SubmitE','Search EMP');

                if($doc != null || $nur!= null || $emp != null){
                    echo "DOCTOR";
                    echo $this->table->generate($queryd);
                    echo "NURSE";
                    echo $this->table->generate($queryn);
                    echo "EMP";
                    echo $this->table->generate($querye);
                }
            ?>
          </div>
          
        </div>

        <div class="clear"></div>
       
        